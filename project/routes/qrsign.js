...
module.exports = function(db) {
	...
	router.post('/sign', function(req, res, next) {
		...
		signManager.insertSignRecord(username, date, classmsg, student_name, student_id);
		res.send('签到成功');
	});
	...
	router.get('/', function(req, res, next) {
		...
		var date = Date.now();
		var svg_string = qr.imageSync('http://172.18.156.227:8000/qr/sign?id=' + id +
	...
	router.get('/record', function(req, res, next) {
		signManager.findAllSignRecords(req.session.user.username).then(function(result) {
			...
			result.forEach(function(val) {
				...
				records[val.date].push(val.student_id+","+val.student_name);
